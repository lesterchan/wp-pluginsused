 ...
 * @coversNothing
 */
class WP_PluginsUsed_Metadata_Test extends WP_PluginsUsed_TestCase {
	...
	protected function plugin_file() {
		return wp_pluginsused_test_read( 'wp-pluginsused.php' );
	}
	...
	protected function readme() {
		return wp_pluginsused_test_read( 'README.md' );
	}
	...
	protected function header_field( $field ) {
		$data = get_file_data( dirname( __DIR__ ) . '/wp-pluginsused.php', array( $field => $field ) );
	...
	public function test_version_matches_everywhere() {
		...
		$this->assertStringContainsString( "define( 'WP_PLUGINSUSED_VERSION', '" . self::VERSION . "' );", $this->plugin_file() );
		$this->assertStringContainsString( 'Stable tag: ' . self::VERSION, $this->readme() );
		$this->assertSame( self::VERSION, WP_PLUGINSUSED_VERSION );
	}
	...
	public function test_every_readme_header_line_keeps_its_line_break() {
		...
		// The first line is the "# WP-PluginsUsed" heading, not a header field.
		$fields = array_slice( $lines, 1 );
		$last   = array_pop( $fields );

		$this->assertCount( 8, $fields, 'Nine header fields, eight of which need the line break.' );
		...
		$this->assertSame( rtrim( $last ), $last, 'The last header line needs no trailing spaces.' );
	}
	...
	public function test_text_domain_is_the_plugin_slug() {
		$this->assertSame( 'wp-pluginsused', $this->header_field( 'Text Domain' ) );
		$this->assertSame( '/languages', $this->header_field( 'Domain Path' ) );
		$this->assertSame( 'wp-pluginsused', WP_PLUGINSUSED_SLUG );
	}

	/**
	 * The licence statement must not contradict itself: the header says GPLv2
	 * or later, so the block below it has to offer the later versions too.
	 */
	 ...
	 * "SECURITY:" or a lowercase "New:" is visible to every reader of it.
	 */
	...
	/**
	 * The raised floors are the break most likely to actually bite, because a
	 * site below them is never offered the update at all.
	 */
	public function test_the_upgrade_notice_states_the_raised_floors() {
	...
	/**
	 * All three filters were public in 1.50 and are gone under those names, so
	 * both spellings of each have to appear where a site owner will look.
	 */
	public function test_the_upgrade_notice_pairs_every_renamed_hook_old_with_new() {
		$notice = substr( $this->readme(), (int) strpos( $this->readme(), '## Upgrade Notice' ) );

		foreach ( array( 'show_version', 'hidden_plugins', 'plugins_used' ) as $hook ) {
			$this->assertStringContainsString(
				'`pluginsused_' . $hook . '`',
				$notice,
				"The Upgrade Notice must name the old pluginsused_{$hook} filter."
			);
			$this->assertStringContainsString(
				'`wp_pluginsused_' . $hook . '`',
				$notice,
				"...beside its replacement, wp_pluginsused_{$hook}."
			);
		}
	}

	/**
	 * Every hook this plugin fires carries its prefix. The three that did not
	 * were renamed in 2.0.0 and dropped rather than deprecated, so a bare one
	 * reappearing means a shim crept back in.
	 */
	...
	public function test_the_plugin_does_not_load_its_own_textdomain() {
		$this->assertStringNotContainsString( 'load_plugin_textdomain', wp_pluginsused_test_source_code() );
	}
	...
	public function test_the_guards_use_the_docblock_form() {
		foreach ( $this->php_directories() as $directory ) {
			// An absolute path: php_directories() returns those, and the plugin
			// root is one of them, so it cannot go through the relative reader.
	...
	public function test_the_gpl_licence_is_shipped() {
		$licence = wp_pluginsused_test_read( 'LICENSE' );
	...
	/**
	 * WP-PluginsUsed ships no JavaScript at all, which is the strongest form of
	 * the house rule: nothing to enqueue, so nothing to depend on jQuery.
	 */
	public function test_no_jquery_is_enqueued() {
		$code = wp_pluginsused_test_source_code();
	...
	public function test_version_row_holds_exactly_plugin_and_db() {
		...
		$markers = get_option( WP_PluginsUsed_Options::VERSION );

		$this->assertIsArray( $markers, 'wp_pluginsused_version must be an array.' );

		$keys = array_keys( $markers );
		sort( $keys );

		$this->assertSame( array( 'db', 'plugin' ), $keys );
		$this->assertSame( WP_PLUGINSUSED_VERSION, $markers['plugin'] );
		$this->assertSame( WP_PLUGINSUSED_DB_VERSION, $markers['db'] );
	}

	/**
	 * The regression guard for the bug this collection's option shape exists to
	 * prevent: a sanitise callback is a function from what the form posted to
	 * what gets stored, and the form never posts a version marker. The moment
	 * one is kept in the settings array, saving the settings screen starts
	 * deciding whether the upgrade routine runs again.
	 */
	public function test_settings_sanitizer_never_stores_version_markers() {
		...
		foreach ( array( 'version', 'db_version', 'versions' ) as $key ) {
			$this->assertArrayNotHasKey(
				$key,
				$clean,
				"A '{$key}' key in the settings array is how the upgrade marker gets lost on save."
			);
		}
	}
	...
	public function test_uninstall_removes_every_option_row() {
		...
		$this->assertSame(
			array(),
			$this->stored_option_names(),
			'uninstall.php must remove every wp_pluginsused_* row.'
		);
	}
}
